...
const showOptionUser = function () {
  $('.option__user--name').text(userDataObject.name.join(' '));
...
userNameInput.addEventListener('change', function () {
  const fullName = $(this).val().toLowerCase().split(' ');
  userDataObject.name = [];
  fullName.forEach(name => {
    userDataObject.name.push(name.charAt(0).toUpperCase() + name.slice(1));
  });
});
userEmailInput.addEventListener('change', function () {
